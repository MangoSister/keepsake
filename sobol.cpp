...
static constexpr uint32_t laine_karras_permutation(uint32_t x, uint32_t seed)
{
    x += seed;
    x ^= x * 0x6c50b47cu;
    x ^= x * 0xb82f1e52u;
...
static constexpr uint32_t nested_uniform_scramble_base2(uint32_t x, uint32_t seed)
{
    x = reverse_bits(x);
...
constexpr uint32_t directions[5][32] = {
...
static uint32_t sobol(uint32_t index, uint32_t dim)
{
    if (dim > 4)
        return 0;
    uint32_t X = 0;
...
{
    constexpr float scale = float(1.0 / (1ull << 32));
